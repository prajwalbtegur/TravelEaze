...
app.post("/register",(req, res) => {
    ...
    pool.executeQuery(sql, function(err, result) {
        if(result.length>0) {
            ...
            id++
            var sql = sqlhelper.insertCommand("user", [id, firstName, lastName, email, password, phoneNumber]);
            console.log(sql);
            pool.executeQuery(sql, function(err, result) {});
            res.redirect("/");
        }
...
app.get("/viewTickets/:userData", (req, res) => {
    ...
    pool.executeQuery(sql, function(err, bus_result) {
        ...
        var sql = sqlhelper.selectCommand("tickets b, tours t", 
                                    ...
                                    "user_id=" + 7 + " and t.tour_id=b.tour_id order by date_of_booking");
        console.log(sql);
...
app.post("/tour_booking/:tourId", (req, res) => {
    console.log("dsadasdsadasdasfasf")
